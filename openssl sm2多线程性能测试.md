<!--
 * @Author: RyanCLQ
 * @Date: 2023-05-28 09:53:52
 * @LastEditors: RyanCLQ
 * @LastEditTime: 2023-05-28 09:55:55
 * @Description: 请填写简介
-->
# openssl sm2多线程性能测试(这个仓库只实现了sm4的并行测试、操作类似，把sm2换成sm4)

> 首先得先从官网下载压缩包：[openssl-3.1.0.zip](https://github.com/openssl/openssl/archive/refs/tags/openssl-3.1.0.zip)

1. 修改文件 sm2_internal_test.c
   ...
   2. 修改后的 sm2_internal_test.c 如后面代码所示

2. 执行命令 `sudo ./config  --prefix=/usr/local/openssl`

3. 在Makefile文件中添加 -fopenmp 编译选项

   ![image-20230525001438872](https://pic-go1.oss-cn-guangzhou.aliyuncs.com/image-20230525001438872.png)
...
#pragma omp parallel for num_threads(thread_num)
        ...
        printf("sig - %d threads: run %d times, total time: %f s, per second run %f tims\n", \
			thread_num, count, (end-begin), count/(end-begin));
    }
#endif
    sig = ossl_sm2_do_sign(key, EVP_sm3(), (const uint8_t *)userid,
